local library = {
	Version = "V.1",
	WorkspaceName = "Index Hub Premium Scripts",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Index Hub Premium Scripts")
							if not fold then
								makefolder("./Index Hub Premium Scripts")
							end
							fold = isfolder("./Index Hub Premium Scripts/Themes")
							if not fold then
								makefolder("./Index Hub Premium Scripts/Themes")
							end
							fold = isfolder("./Index Hub Premium Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Index Hub Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Index Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Index Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Index Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Index Hub Premium Scripts" then
		library.WorkspaceName = convertfilename(windowName, "Index Hub Premium Scripts")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Index Hub Premium Scripts") then
						makefolder("./Index Hub Premium Scripts")
					end
					local common_string = "./Index Hub Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Index Hub Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Index Hub Premium Scripts Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
